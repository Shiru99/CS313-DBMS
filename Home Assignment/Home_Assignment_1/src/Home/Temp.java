	...
	public static void main(String[] args) throws IOException {
		...
		try (Connection connection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/university",
			...
			while ((lineText = lineReader.readLine()) != null) {
				...
				PreparedStatement checkCourse = connection
						.prepareStatement("SELECT COUNT(*) FROM course WHERE course_id = ?");
				...
				if (countCourse.getInt("count") > 0) {
					...
					if (countStudent.getInt("count") > 0) {
						PreparedStatement updateGrade = connection
								.prepareStatement("UPDATE takes SET grade = ? WHERE id = ?");
						PreparedStatement updateTotalStudentCredits = connection
								.prepareStatement("UPDATE student SET tot_cred = ? WHERE id = ?");
						...
						if (!grade.equals("F")) {
							...
							PreparedStatement getCredits = connection
									.prepareStatement("SELECT credits FROM course WHERE course_id = ?");
							...
							PreparedStatement getTotalStudentCredits = connection
									.prepareStatement("SELECT tot_cred FROM student WHERE id = ?");
